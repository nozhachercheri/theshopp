...
class OrderController extends Controller
{
    ...
    public function FindOrderByUser()
    {
        
          
      

        $order=Order::where('user_id',auth('api')->user()->id)->pluck('id');
        return response()->json([
            'success' => true,
            'message' => translate('dddd'),
            'data' => $order
        ], 200);
    }

    public function FindProductByOrder($IdOrder)
    {
        
        $ordersetail=OrderDetail::where('order_id',$IdOrder)->pluck('product_id');
        return response()->json([
            'success' => true,
            'message' => translate('dddd'),
            'data' => $ordersetail
