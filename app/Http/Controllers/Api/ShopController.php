...
class ShopController extends Controller
{
    ...
    {
       $id=Shop::where('slug',$slug)->pluck('id');
       
        $brandbr=ShopBrand::where('shop_id',$id)->pluck('brand_id');
        return $brandbr;
    }
    

    public function indexAllId()
    {
    ...
    {
        $shop = filter_shops(Shop::where('id', $id))->first();
    ...
    public function show_by_name(Request $request)
    {
        ...
        //   $shop->where(function ($q) use ($search_keyword) {
        //       foreach (explode(' ', trim($search_keyword)) as $word) {
        //           $q->where('name', 'like', '%' . $word . '%');
        //       }
        //   });
       
        $shop =Shop::where(function ($q) use ($search_keyword) {
            foreach (explode(' ', trim($search_keyword)) as $word) {
                $q->where('name', 'like', '%' . $word . '%');
            }
        })->first();
  
       //$shop =Shop::where('name', 'like', '%'.$request->keyword.'%')->first();
    ...
    // public function show_by_name(Request $request)
    // {
    //     $shop = filter_shops(Shop::where('name', $request->name))->first();
    ...
    //     }
    // }
    public function showAll_by_id($id)
    {
        $shop = filter_shops(Shop::where('id', $id)->withCount(['products', 'reviews'])->with('categories'))->get();
