    ...
    <div class="row">
        <div class="col-lg-8 mx-auto">
            <div class="card">
                ...
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('shop_settings.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group row">
                            <label class="col-sm-3 col-from-label">{{translate('Minimum order amount')}}</label>
                            <div class="col-sm-9">
                                <input type="number" class="form-control" name="min_order" min="0" step="0.01" value="{{ auth()->user()->shop->min_order }}" required>
                                <small class="text-muted">{{ translate('Customer need to purchase minimum this amount of admin shop products to place an order.') }}</small>
                            </div>
                        </div>
                        <div class="text-right">
      						<button type="submit" class="btn btn-primary">{{ translate('Update') }}</button>
